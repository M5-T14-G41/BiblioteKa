...
class LoanSerializer(serializers.ModelSerializer):
    def create(self, validated_data):
        user_loans = Loan.objects.filter(
            user_id=validated_data["user_id"], is_returned=False)
        copy_loans = Loan.objects.filter(
            copy_id=validated_data["copy_id"], is_returned=False).exists()
        not_returned = []
        user_status = {"banned": False}

        if copy_loans:
            raise ValidationError(
                {"message": "This copy has already been loaned"})

        for loan in user_loans:
            convert_devolution_date = dt.strptime(
                str(loan.devolution_date), "%Y-%m-%d")
